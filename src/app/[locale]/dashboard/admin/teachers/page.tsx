...
export default function AdminTeachersPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'verified' | 'featured' | 'inactive'>('all');
  ...
  const visible = useMemo(() => teachers.filter((teacher) => {
    ...
    const state = filter === 'all' || (filter === 'verified' && teacher.verified) || (filter === 'featured' && teacher.featured) || (filter === 'inactive' && !teacher.is_active);
  ...
  return (
    <AdminShell>
      ...
      <div className="mb-5 flex flex-col gap-3 border border-black/10 bg-[#f5f2e9] p-3 md:flex-row md:items-center"><label className="flex flex-1 items-center gap-3 border border-black/10 px-3"><Search className="h-4 w-4 text-black/30" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search name, email, subject…" className="min-w-0 flex-1 bg-transparent py-3 text-sm outline-none" /></label><div className="flex gap-1 overflow-x-auto">{(['all', 'verified', 'featured', 'inactive'] as const).map((item) => <button key={item} onClick={() => setFilter(item)} className={`shrink-0 px-4 py-3 text-xs font-semibold capitalize ${filter === item ? 'bg-[#171813] text-white' : 'text-black/45 hover:bg-black/5'}`}>{item}</button>)}</div></div>
